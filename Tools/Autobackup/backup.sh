...
if [ ! -d "$backup_dir" ]; then
 ...
 mkdir $backup_dir/para_$backup_date
fi

cp -R /home/metro/metro_center/datafile/end/ACC/send/$da $backup_dir/data_$backup_date

cp -R /home/metro/metro_center/parafile/curparafile $backup_dir/para_$backup_date

tar -czvf $backup_dir/data_$backup_date.tar.gz $backup_dir/data_$backup_date

tar -czvf $backup_dir/para_$backup_date.tar.gz $backup_dir/para_$backup_date
...
backup_para_size=$(du -h $backup_dir/para_$backup_date.tar.gz | awk '{print $1}')

echo "$backup_date Backup Completed: data_$backup_date.tar.gz {$backup_size}" >> $backup_dir/backup_$da.log

echo "$backup_date Backup Completed: para_$backup_date.tar.gz {$backup_size}" >> $backup_dir/backup_$da.log
