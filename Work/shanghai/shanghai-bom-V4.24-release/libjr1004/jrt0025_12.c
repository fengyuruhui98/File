...
void	WCalMac(UWORD nLenIn,UBYTE *pszBufIn,UBYTE *pszInitData,UBYTE *pszKey,UBYTE *pszMAC,UBYTE	bTriDes)
{
	...
	UWORD		nLenTotal;		//	\B2\B9\D7\E3\D7\EE\BA\F3縗B8\F6\CA\FD\BE縗E9\B5縗A4\B6縗A3
...
void init_tagList()
{
	...
	sprintf(tag_PPSE[0].sDescribe,"FCI模板");
	tag_PPSE[0].min_Len=0;
	tag_PPSE[0].max_Len=0;	//min.max == 0  变长
	...
	sprintf(tag_PPSE[2].sDescribe,"FCI专用模板");
	...
	sprintf(tag_PPSE[3].sDescribe,"FCI发卡行自定义数据");
	...
	sprintf(tag_PPSE[4].sDescribe,"目录入口");
	...
	sprintf(tag_PPSE[6].sDescribe,"应用标签");
	...
	sprintf(tag_PPSE[7].sDescribe,"应用优先指示器");
...
int PPSE_Proc_L2(UBYTE*  inBuf,int nInLen,UBYTE* uAID,int* uLen_AID)
{
	...
	{
	#ifdef _DEBUG_PPSE_SEL_
		fprintf(stderr,"PPSE,uTmp %d字节 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>:",uLen_Tmp);
		...
		#ifdef _DEBUG_PPSE_SEL_
			fprintf(stderr,"PPSE,跳过AID ,uTmp %d字节 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>:",uLen_Tmp);
			for(i=0;i<uLen_Tmp;i++)
			{
				fprintf(stderr,"%02X ",uTmp[i]);
			}
		...
		{
			fprintf(stderr,"PPSE,87:有条件:%02X %02X %02X",tag_PPSE[4].uValue[tag_PPSE[5].uLength+2+tag_PPSE[6].uLength],tag_PPSE[4].uValue[tag_PPSE[5].uLength+2+tag_PPSE[6].uLength+1],tag_PPSE[4].uValue[tag_PPSE[5].uLength+2+tag_PPSE[6].uLength+2]);
	...
	#ifdef _DEBUG_PPSE_SEL_
		fprintf(stderr,"tag_PPSE[4].uLength+2= %d字节 uLen_Tmp=%d,",tag_PPSE[4].uLength+2,uLen_Tmp);
		for(i=0;i<uLen_Tmp;i++)
			fprintf(stderr,"%02X ",uTmp[i]);
		fprintf(stderr,"\r\n--------------------------------------------------");
	#endif
...
//#endif
	...
	fprintf(stderr,"\x0d\x0a gSDFORAUTH(%d字节):\n",uLen_gSDFORAUTH);
...
int GenPDOL(UBYTE* uOutData,int* nLen_OutData)
{
	...
	for(i=0;i<uLen_g9f38;i++)
	{
		...
		fprintf(stderr,"生成9f34(%d字节):",l_tlv);
	#endif
			...
			if(g9f38[i+1]==0x41)
			{
				...
				l_tlv = g9f38[i+2];
	#ifdef _DEBUG_Parse_AIDRESP_
		fprintf(stderr,"生成9f41(%d字节):",l_tlv);
	#endif
		...
		if(g9f38[i]==0x81 && g9f38[i+1]==0x04 )
		{
			//生成81 
	#ifdef _DEBUG_Parse_AIDRESP_
		fprintf(stderr,"生成81(%d字节):",g9f38[i+1]);
	#endif
			l_tlv = g9f38[i+1];
		...
		if(g9f38[i]==0x5f)
		{
			...
			if(g9f38[i+1]==0x30)
			{
				...
				l_tlv = g9f38[i+2];
	#ifdef _DEBUG_Parse_AIDRESP_
		fprintf(stderr,"生成5f30(%d字节):",l_tlv);
	#endif
		...
		if(g9f38[i]==0xdf && g9f38[i+1]==0x1a )
		{
				...
				l_tlv = g9f38[i+2];
	#ifdef _DEBUG_Parse_AIDRESP_
		fprintf(stderr,"生成df1a(%d字节):",l_tlv);
	#endif
		...
		if(g9f38[i]==0x9f && g9f38[i+1]==0x80 )
		{
				...
				l_tlv = g9f38[i+2];
	#ifdef _DEBUG_Parse_AIDRESP_
		fprintf(stderr,"生成9f80(%d字节):",l_tlv);
	#endif
	...
	for (i=0; i<count; ++i)
	{
		...
		if(memcmp(tlv[i].tag.data,"\x8f",tlv[i].tag.size)==0)
		{
			...
			memcpy(&g8f,tlv[i].val.data,tlv[i].val.size); 
				...
				fprintf(stderr,"公钥索引g8f=%02X \x0d\x0a",g8f);
			#endif
		}
...
void GetTagFrmTLV(UBYTE* uTlv,int nLen_TLV)
{
	...
	{
		#ifdef _DEBUG_GetTagFrmTLV_
			fprintf(stderr,"TLV解构,uLen_tagValue=%02x,nLen_Tmp=%d,i=%d:",uLen_tagValue,nLen_Tmp,i);
		...
		for(j=0;j<TAGNUMBER;j++)
		{
			if(Tag_All[j].uName[0]==uTmp[i])
			{
				if(Tag_All[j].uLength==1)
				{
					...
					#ifdef _DEBUG_GetTagFrmTLV_
						fprintf(stderr,"找到标签:tag %02X",Tag_All[j].uName[0]);
					#endif
					break;
				}
				...
				if(Tag_All[j].uLength==2 && Tag_All[j].uName[1]==uTmp[i+1])
				{
					...
					#ifdef _DEBUG_GetTagFrmTLV_
						fprintf(stderr,"找到标签:tag %02X%02X",Tag_All[j].uName[0],Tag_All[j].uName[1]);
					#endif
					break;
				}	
			}	
		...
		//uTmp指针移动
		#ifdef _DEBUG_GetTagFrmTLV_
			fprintf(stderr,"uTmp指针移动前,uLen_tagValue=%02X,nLen_Tmp_Total=%d",uLen_tagValue,nLen_Tmp_Total);
		#endif	
		...
		i=0;
		#ifdef _DEBUG_GetTagFrmTLV_
			fprintf(stderr,"uTmp指针移动后,uLen_tagValue=%02X,nLen_Tmp_Total=%d",uLen_tagValue,nLen_Tmp_Total);
		#endif
...
*/
		...
		if(gCardBusinessType==0x90)
		{
			...
			g_MECCardInfo->CardType=R_Apdu.data[28];//卡类型
			memcpy(g_MECCardInfo->Validdate,R_Apdu.data +24,sizeof(g_MECCardInfo->Validdate));//有效期
			...
			memcpy(gBalance,R_Apdu.data,4);//电子钱包余额
			...
			if(nRetCode != OK)
				return nRetCode;
			//20130722
			g_MECCardInfo->Flag=R_Apdu.data[12+0];//范围标志(1）
			memcpy(g_MECCardInfo->LineBitMap,R_Apdu.data+12+1,sizeof(g_MECCardInfo->LineBitMap));//+线路位图（6）+
			memcpy(g_MECCardInfo->StationBitMap,R_Apdu.data+12+7,sizeof(g_MECCardInfo->StationBitMap));//+车站位图（12）
			...
			memset(gF19_Thr,0x00,sizeof(gF19_Thr));
			memcpy(gF19_Thr,R_Apdu.data,3);//复合消费标志(0x09) + 记录长度 + 应用锁定标志位  20130613
...
/*20131117
		...
		if(g_TP_Head->LCYH_Flag[0]==1)
		{
			...
			{
			#ifdef _DEBUG_L2T_
				fprintf(stderr,"交易终止  AID:uRetCode=%04X,%d",nRetCode,nRetCode);
			#endif
				return (nRetCode) ;
			}
		}
		...
		{
		#ifdef _DEBUG_L2T_
			fprintf(stderr,"交易终止,没有匹配的非接触交易路径(AID)");
		#endif
			return ERR_NOAIDPATCH;
		}
	*/		
	...
	gInt2Bytes(gn9f79- gndf63,g_CardInfo->Balance);//卡上实际余额
	memcpy(gBalance,g_CardInfo->Balance,4);//电子钱包余额	
	gShort32Bytes(gndf62,g_CardInfo->Overdrawlimited);//透支限额
#ifdef _DEBUG_GETINFO_
	printf("\n\r  gBuf0221:\n\r");
	for(i=0;i<41;i++)
	{
...
int ReadRecords()
{
	...
	for(i=0;i<nLen_AFL/4;i++)
	{
...
#ifdef _DEBUG_L2T_
		fprintf(stderr,"根据AFL读,SFI=%02X(%02X)",AFL[i*4],AFL[i*4] | 0x04);
#endif
		if(uRecno_Begin==0x00)
		{
		#ifdef _DEBUG_L2T_
			fprintf(stderr,"交易终止,第1 个记录的记录号（不能为0）");
		#endif
		...
		{
		#ifdef _DEBUG_L2T_
			fprintf(stderr,"交易终止,SFI=%02X",SFI);
		#endif
			return ERR_SFI31 ;
		}	 
		...
		{
		#ifdef _DEBUG_L2T_
			fprintf(stderr,"交易终止,最后一个记录的记录号错误");
		#endif
		...
		{
		#ifdef _DEBUG_L2T_
			fprintf(stderr,"交易终止,参与数据认证记录的记录号错误");
		#endif
...
#ifdef _DEBUG_L2T_
	fprintf(stderr,"根据AFL读,SFI=%02x,uRecno=%02X,uRecno_Begin=%02X,uRecno_End=%02X, auth_flag=%02X",SFI,uRecno,uRecno_Begin,uRecno_End,auth_flag);
#endif
			//测试闪卡
	...
	ReaderResponse(csc_comm, 0x00, 0xF0, pfbuf, 4);
			...
			{
			#ifdef _DEBUG_L2T_
				fprintf(stderr,"交易终止:uRetCode=%04X",nRetCode);
			#endif
				//fprintf(stderr,"\r\n PASSCARD:uRetCode=%d,i=%d,%d,%d,%d \r\n",nRetCode,i,nLen_AFL/4-1,uRecno,uRecno_End);
				//读最后一条记录超时，闪卡错误
			...
			{
			#ifdef _DEBUG_L2T_
				fprintf(stderr,"交易终止,记录模板错误[%02X]",R_Apdu.data[0]);
			#endif
				return ERR_MODEREC ;
			}		
			//记录长度
			if(R_Apdu.data[0]==0x70)
			{
				if(R_Apdu.data[1]==0x81)
				{
					...
					{
					#ifdef _DEBUG_L2T_
						fprintf(stderr,"交易终止,记录长度字节错误[%02X]",R_Apdu.data[2]);
					#endif
						return ERR_LENBYTE ;
					}
				}
				...
				{	
					...
					{
					#ifdef _DEBUG_L2T_
						fprintf(stderr,"交易终止,记录长度字节错误[%02X]",R_Apdu.data[1]);
					#endif
						return ERR_LENBYTE ;
					}
				}		
			...
			#ifdef _DEBUG_L2T_
				fprintf(stderr,"参与认证静态数据,uLen_gSDFORAUTH=%d,l_tlv=%02X(hex),auth_flag=%02x(hex):",uLen_gSDFORAUTH,l_tlv,auth_flag);
			...
			{
			#ifdef _DEBUG_L2T_
				fprintf(stderr,"交易终止,磁条2 等效数据重复.");
				return ERR_DBL57;
			#endif
			}
	...
	for(i=0;i<uLen_gSDFORAUTH;i++)
	{
		if(gSDFORAUTH[i]==0x57)
		{
			...
			uLen_g57=l_tlv;
		#ifdef _DEBUG_L2T_
			fprintf(stderr,"取 g57,l_tlv=%02X:",l_tlv);
			for(j=0;j<l_tlv;j++)
...
unsigned char GetOddParityBit(unsigned char Data)//奇校验 
{
...
int UpdateCAppCache(int nTrade,char *inbuf)
{
	...
	if(nTrade==10041)
	{
		...
		{
			memcpy(g_Data0221-> MonthAmount,"\x00\x00\x00",3);	//当月轨道交通累计金额
			memcpy(g_Data0221->MonthCount,"\x00\x00",2);	//当月轨道交通累计次数
		}

		//正常进站 :Bit3： 0表示正常进站      1表示出站换乘进站
		//if(inbuf[10] & 0x08==0x00)
		//printf("\n\r Entry Flag=%02X,inbuf10=%02X\n\r",gR_Entry->Flag,inbuf[10]);
		if((gR_Entry->Flag & 0x08)==0x00)
		{
			g_Data0221->Status =0x00;//清
		...
		{
			//换乘
			g_Data0221->Status = (g_Data0221->Status | 0x10) ; //bit4 =1表示出站换乘进站			
			g_Data0221->Status = (g_Data0221->Status & 0x3E);//清bit7 bit6
		}
		//超时 20130722
		...
		memcpy(g_Data0221->LastTime,gR_Entry->TradeTime,sizeof(g_Data0221->LastTime));//末笔时间
		memcpy(g_Data0221->LastTermNo,gTerminalNo,sizeof(g_Data0221->LastTermNo));//末笔终端
	...
	if(nTrade==10042)
	{
		...
		//若交易类型为0、1、9、10则“出站交易时间”、“出站轨道交通终端编号”、“末笔交易时间”、“末笔轨道交通终端编号”都被改写；
		...
		//}
		memcpy(g_Data0221->LastTime,gR_Exit->TradeTime,sizeof(g_Data0221->LastTime));//末笔时间
		memcpy(g_Data0221->LastTermNo,gTerminalNo,sizeof(g_Data0221->LastTermNo));//末笔终端
		...
		gShort2Bytes(nAmount,g_Data0221->Amount);;//实际交易金额=实际交易金额+交易金额
...
//printf("\n\r nTotal= %d \n\r",nTotal);
		...
		nTotal++;
		gShort2Bytes(nTotal,g_Data0221->MonthCount);	//当月累计次数
...
//printf("\n\r nTotal= %d ,g_Data0221->MonthCount=%02X %02X\n\r",nTotal,g_Data0221->MonthCount[0],g_Data0221->MonthCount[1]);
		...
		g_Data0221->Status = (g_Data0221->Status & 0xEF) ; //清出站换乘进站flag,20130724	
		if(GetOddParityBit(g_Data0221->Status)==0)
			g_Data0221->Status = (g_Data0221->Status | 0x01);
	...
	if(nTrade==10043)
	{
		...
		memcpy(g_Data0221->LastTermNo,gTerminalNo,sizeof(g_Data0221->LastTermNo));//末笔终端
		//Bit2:0表示不改写的实际交易金额 1表示对实际交易金额清零
		if((gR_Update->TimeFlag & 0x04)==0x04)
		{
			memset(g_Data0221->Amount,0x00,2);
		} 	
		//Bit1:0表示不改写出站交易日期/时间域 1表示用参数中的交易日期/时间改写出站交易日期/时间域
...
int L2Process(int nTrade,char *inbuf,char *outbuf)
{
	...
	//if(nRetCode !=  OK)
	//{
	//	fprintf(stderr,"取读卡器时间失败:uRetCode=%04X",nRetCode);
	//	return (nRetCode) ;
	//}
#ifdef _DEBUG_L2T_
	fprintf(stderr,"读卡器时间:");
	for(j=0;j<6;j++)
		fprintf(stderr,"%02X ",uTmp[j]);
	fprintf(stderr,"\n\r");
#endif
...
int MECProcess(int nTrade,char *inbuf,char *outbuf)
{
	...
	if(nTrade==10041)
		memcpy(uTmp+6,"\x00\x00\x00\x00",4);//交易金额
	if(nTrade==10042)
	{
		uTmp[6]=0x00;
	...
	memcpy(uTmp+11,inbuf,7);//交易日期时间
	uTmp[18]=uKeyVer;
	uTmp[19]=uAlgoID;
	memcpy(uTmp+20,gAppSN,8) ;//应用序列号：卡片ADF下0015文件中的应用序列号
	...
	C_Apdu.data[2] = gBuf19[0];//复合消费标志
	//memcpy ( C_Apdu.data +5 , "\x09\x3e\x00",3);	

	memset(uTmp,0x00,sizeof(uTmp));	
	memcpy ( uTmp , gBuf19,sizeof(gBuf19));
	//根据交易类型及gBuf19，写19文件数据
	g_DataMEC19 = (F_DataMEC19*)uTmp;
	if(nTrade==10041)
	{
		if(memcmp(g_DataMEC19->EntryTime,inbuf,3)!=0)
		{
			memcpy(g_DataMEC19-> MonthAmount,"\x00\x00\x00",3);	//当月轨道交通累计金额
			memcpy(g_DataMEC19->MonthCount,"\x00\x00",2);	//当月轨道交通累计次数
		}
		...
		memcpy(g_DataMEC19->LastTime,inbuf,7);	//末笔时间
		memcpy(g_DataMEC19->LastTermNo,gTerminalNo,4);//末笔终端
	}
	//出站
	if(nTrade==10042)
	{
		...
		//if(gR_Exit->TradeType==0 || gR_Exit->TradeType==1 || gR_Exit->TradeType==9 || gR_Exit->TradeType==10)
		//{
			memcpy(g_DataMEC19->ExitTime,inbuf,7);	//出站时间
			memcpy(g_DataMEC19->ExitTermNo,gTerminalNo,4);//出站终端
		...
		g_DataMEC19->Status = (g_DataMEC19->Status & 0xEF) ; //清出站换乘进站flag,20130724
		if(GetOddParityBit(g_DataMEC19->Status )==0)
			g_DataMEC19->Status  = (g_DataMEC19->Status  | 0x01);
		...
		nAmount = nAmount +nYJE;
		gShort2Bytes(nAmount,g_DataMEC19->Amount);;//实际交易金额=实际交易金额+交易金额
	...
	if(nTrade==10044 )
	{
		g_DataMEC19->AppLockFlag=0x01;//suo锁卡标志
	}
	...
	memcpy ( C_Apdu.data +5 , uPSAMCounter,4);//终端交易流水号
	memcpy ( C_Apdu.data +9 , inbuf,7);//交易日期时间;
...
int pf_mobile_normal_ticket_exit(char *inbuf,char *outbuf)
{
	...
	if(ret==0)
		...
		GenOutBuf(0,outbuf);
	//SaveTransData(outbuf+1);  需要根据出站交易修改 20130724
	if(ret>MAXNOCARDRETCODE)
		ret = ret * -1;
#ifdef _DEBUG_NORMALEXIT_
	printf("\r\n L2Process return = %d \r\n",ret);
...
int pf_mobile_staff_ticket_get_record(char  *inbuf,char  *outbuf)
{
	...
	for(i=1;i<=8;i++)
	{
		...
		{
			uRecordNo = i;
			memcpy ( C_Apdu.data , "\x00\xb2\x01\xc4\x17",5);//读18文件
			C_Apdu.data[2]=uRecordNo;
			nRetCode = PBOC_apdu_Ext(&C_Apdu, &R_Apdu );
