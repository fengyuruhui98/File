...
void sha_update(SHA_INFO *sha_info, BYTE *buffer, int count)
{
    ...
    clo = T32(sha_info->count_lo + ((UNLONG) count << 3));
    if (clo < sha_info->count_lo) {
	++sha_info->count_hi;
    }
    sha_info->count_lo = clo;
    sha_info->count_hi += (UNLONG) count >> 29;
    if (sha_info->local) {
...
{
    int count;
    UNLONG lo_bit_count, hi_bit_count;

    lo_bit_count = sha_info->count_lo;
    hi_bit_count = sha_info->count_hi;
    //hi_bit_count = sha_info->count_lo;
    //lo_bit_count = sha_info->count_hi;
    count = (int) ((lo_bit_count >> 3) & 0x3f);
    ((UBYTE *) sha_info->data)[count++] = 0x80;
    if (count > SHA_BLOCKSIZE - 8) {
	memset(((UBYTE *) sha_info->data) + count, 0, SHA_BLOCKSIZE - count);
	sha_transform(sha_info);
	memset((UBYTE *) sha_info->data, 0, SHA_BLOCKSIZE - 8);
    } else {
	memset(((UBYTE *) sha_info->data) + count, 0,
...
void sha(const UBYTE* DataIn,unsigned int DataInLen,UBYTE* DataOut)
{
	unsigned int i,k,m;
	SHA_INFO sha_info;
	...
	for(i=0;i<k;i++) {
		sha_update(&sha_info, (UBYTE*)&DataIn[i*SHA_BLOCKSIZE], SHA_BLOCKSIZE);
	}
	if(m!=0)
		sha_update(&sha_info, (UBYTE*)&DataIn[i*SHA_BLOCKSIZE], m);

	sha_final(DataOut, &sha_info);
}
