...
//#define DEBUG_WHITE_4020
//#define DEBUG_CERT_4026
...
int i;
	...
	{
		if( (family == tpCert4026.Cert_Content_val[i].type) && (index == tpCert4026.Cert_Content_val[i].index) && (version == tpCert4026.Cert_Content_val[i].ver))
		{
			//printf("\nfamily:%02x 4026type:%02x\n",family,tpCert4026.Cert_Content_val[i].type);
			//printf("\nindex:%02x 4026index:%02x\n",index,tpCert4026.Cert_Content_val[i].index);
			//printf("\nversion:%02x 4026version:%02x\n",version,tpCert4026.Cert_Content_val[i].ver);
			memcpy(outbuf, &tpCert4026.Cert_Content_val[i].type, sizeof(struct Cert_Content));
			break;
		}
	}
	if ( i >= tpCert4026.cert_head.ContentNumber ){
		//printf("\nzdd:4026 can not find");
		return 0xff;
	}
...
int i;
	...
	for(i = 0; i < tpWhite4020.white_head.ContentNumber; i++)
	{
		if( (memcmp(os_pb_id, tpWhite4020.White_Content_val[i].CardIssuer, 4) == 0) && (memcmp(os_orgnization, tpWhite4020.White_Content_val[i].IIN, 4) == 0) )
		{
...
//				printf(" %02X",cmd_buf[4+i]);
		...
		{
#ifdef DEBUG_WHITE_4020			
			printf("\ncontinuous frame frame..1: %d  curframe..1: %d filename..1: %s\n", shbyte2, eod_download.curFrame, eod_download.filename);
#endif
			//first check the frame continuous
...
//				printf(" %02X",cmd_buf[4+i]);
		...
		{
#ifdef DEBUG_WHITE_4020			
			printf("\ncontinuous frame frame..2: %d  curframe..2: %d filename..2: %s\n", shbyte2, eod_download.curFrame, eod_download.filename);
#endif
			if((shbyte2 | 0x80) == 0x80)		//判断第一包数据是否为最后一包
			{
				sprintf(eod_download.filename, "5020");
...
#ifdef DEBUG_WHITE_4020
			printf("\nshmt_read_4020 end2...");
			printf("\n\neod_download.filename  %s\n",eod_download.filename);
#endif
			shmt_read_4020(eod_download.filename);
			
/*			//�����汾У�飺2019-06-18 ������صĲ����汾С�ڵ��ڼ��в����汾����ܾ����أ������ش�����0x03;
#ifdef DEBUG_WHITE_4020
			printf("\ntemp_4020...1 :%02x%02x%02x%02x\n",temp_4020[0],temp_4020[1],temp_4020[2],temp_4020[3]);
			printf("ParaVersion...1 :%04x\n",tpWhite4020.white_head.ParaVersion);
#endif			
			LongToByte(tpWhite4020.white_head.ParaVersion,temp_version);
			//memcpy(temp_version,&tpWhite4020.white_head.ParaVersion,4);
#ifdef DEBUG_WHITE_4020
			printf("temp_version...1 :%02x%02x%02x%02x\n",temp_version[0],temp_version[1],temp_version[2],temp_version[3]);
#endif
			//if(memcmp(&temp_version,&tpWhite4020.white_head.ParaVersion,4)<=0)
			if(memcmp(temp_4020,temp_version,4)<=0)
			{
				//memcpy(buf_4020,&tpWhite4020.white_head.ParaVersion,4);
				remove(eod_download.filename);
				memset(&temp_version,0x00,4);
				out_buf[0] = 0x03;
				*out_len = 1;
				return;
			}	*/
			...
			for(i=0;i<20;i++)
			{
				printf("%02x ",temp_4020[i+8]);
			}
			printf("\n");
#endif
			ret=check_sha1(temp_4020);
#ifdef DEBUG_WHITE_4020
			printf("\ncheck_sha1...ret :%d\n",ret);
#endif
			if(ret != 0)
			{
				...
				*out_len = 1;
				...
				//memcpy(buf_4020,&tpWhite4020.white_head.ParaVersion,4);
				//printf("\nbuf_4020...:%02x%02x%02x%02x\n",buf_4020[0],buf_4020[1],buf_4020[2],buf_4020[3]);
...
//#ifdef DEBUG_WHITE_4020
//			printf("\ntemp_4020...2 :%02x%02x%02x%02x\n",temp_4020[0],temp_4020[1],temp_4020[2],temp_4020[3]);
//			printf("\nbuf_4020...2 :%02x%02x%02x%02x\n",buf_4020[0],buf_4020[1],buf_4020[2],buf_4020[3]);
//#endif
				out_buf[0]= 0;
				*out_len = 1;
			}
...
#endif
			//first check the frame continuous
			if(shbyte2 != eod_download.curFrame + 1)//�жϵ�ǰ���Ƿ�����
			{
				chret = CE_CONTINUE_MULTIFRAME;	//not continuous frame
				out_buf[0]= 0x02;
				*out_len = 1;
...
//					printf(" %02X",cmd_buf[4+i]);
			...
			ret = memcmp(shaRes, &filebuf[12], 20);
			free(filebuf);
			if(ret != 0)
			{
...
#ifdef DEBUG_WHITE_4020
		printf("active_eod_file...");
#endif
	*lngver = 0;
	ret = FileisExist("./", filename, chnewfile);
	//printf("\nFileisExist...ret 1: %d\n",ret);
	if(ret != 0)
		return ret;
	//first check new eod file according to the md5
	sprintf(newfilepath, "./%s", chnewfile);
	//*lngver = atol(&chnewfile[ver_len]);
	fl = fopen(newfilepath, "rb");
		fread(&eod_ver, 1, sizeof(eod_ver), fl);
	fclose(fl);
	...
	//find the current eod file and delete
	//maybe find all the same eod file and delete it.??
	ret = FileisExist("./", filename, chfile);
	//printf("\n\nFileisExist...ret 2: %d\n",ret);
	if(ret == 0)
	{
		sprintf(filepath, "./%s", chfile);
...
int check_sha1(char *inbuf)
{
	...
	ByteToLong(&Number,&temp_4020[4]);
	DataLength = Number * 16;
	printf("\nSHA Check...");
	sha(inbuf+28,DataLength,DataOut);
		...
		printf("\n");
#endif
	if (memcmp(DataOut,&temp_4020[8],20)!=0) 
	{
		return -1;	
	}
