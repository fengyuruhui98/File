...
int 	i, j, ret;
char 	filename[40];
unsigned char name_4016[10],name_4020[10];
...
time_t lngtime;
...
#ifdef _DEBUG_BOM_
	debug_printf("\nshanghai soft Ver = %02X\n",gVersion);
	debug_printf("shmt_read_pf4016 start1...\n");
#endif
	//4020
	EOD_4020_WhiteList();
	EOD_4026_Cert();
...
#ifdef _DEBUG_BOM_
	debug_printf("shmt_read_pf4016 end1...\n");
#endif
	//2019-06-10
	Init_CRC32_Table();
	...
	sem_init(&g_tmrwait,0,1);//  extern int sem_init __P ((sem_t *__sem, int __pshared, unsigned int __value));　
...
//	parity_set(zte_comm, 8, 1, 'n');
	...
	printf("open comm %s handle is %d\n", argv[1], ret);
	if(ret < 0) return -1;
	csc_comm = ret;
