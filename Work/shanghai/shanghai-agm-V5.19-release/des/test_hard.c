...
char reader_test_eeprom(void)
{
	...
	int eepromsize, eepromstep;
		...
		for(j = 0; j < (eepromsize / eepromstep); j++)
		{
			...
			{
					#ifdef DEBUG_PRINT
								printf("\nErr:读EEPROM失败,add=%x!", j * eepromstep);
					#endif
				return 0x10;
	  		}
			...
			{
					#ifdef DEBUG_PRINT
								printf("\nErr:写EEPROM失败,add=%x!", j * eepromstep);
					#endif
	  			return 0x20;
	  		}
...
char reader_test_rtc(char write_control, unsigned char *time_bcd, unsigned char *out_len)
{
		...
		char inbuf[100];
			...
			if(write_control)
			{
				...
				#ifdef DEBUG_PRINT
						printf("\nOK:写时钟成功 %02x%02x%02x %02x:%02x:%02x.",inbuf[0],inbuf[1],inbuf[2],inbuf[3],inbuf[4],inbuf[5]);
				#endif
			}
			...
			#ifdef DEBUG_PRINT
				printf("\nOK:读时钟成功 %02x-%02x-%02x %02x:%02x:%02x.",inbuf[0],inbuf[1],inbuf[2],inbuf[3],inbuf[4],inbuf[5]);
			#endif
			if(write_control)
			{
...
char reader_test_littlesam(char channel, unsigned char *out_buf, unsigned char *out_len)
{
		...
		unsigned char outbytes;
			...
			*out_len = 1;
			...
			{
				#ifdef DEBUG_PRINT
						printf("\nOK:SAM%02d测试成功!", samindex);
				#endif
		  	}else
		  	{
				#ifdef DEBUG_PRINT
				  		printf("\nErr:SAM%02d测试失败!", samindex);
				#endif
		  		out_buf[0] = 0x12;
				return 0xff;
			}
			...
			*out_len = 1;
			...
			for(i = 0; i < try_times; i++)
			{
				...
				{//M1
					...
					//M1 end
								//ats
					//memcpy(Key_test,"\xfc\x61\xcb\xb5\x37\x51",6);   //正式
					//ret=mcmh_read_simple(1,buf,1,0);
					//if(ret!=0)
					//CPU
					if(1)
					{ 
						//memcpy(Key_test,"\x70\xa4\xf4\x82\x01\x6d",6);   //测试
...
//							}else
				...
				next:		
					...
					//for(j = 0;j < 3;j++)
					//{
							ret = mifpro_icmd0(7,inbuf, outbuf);
							if(ret != 0)
							{
								#ifdef	DEBUG_PRINT
