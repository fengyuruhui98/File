...
label_m1:
	...
	bgSHpsamIndex=SAMLOCATION_5;			//指定SAM卡座
	ret=getcardinfo();	
	if(ret==0) 
	{
...
int ret;
	...
	buf[21] = cardversion;  //卡版本号（地区号）
	if(NormalCard==COMMCARD){
		memcpy(&buf[22],info,5);//换乘行业信息、换乘时间
		memcpy(&buf[27],&leisure[2],3);//累计乘车金额
  }
	else{
		buf[22] = DayToAllow;
	...
	memcpy(&buf[30],leisure,2);//累计乘车次数
	buf[32] = gMetroIoFlagBk;//备份进出状态1
	buf[33] = gMetroIoFlagBkValid;//备份进出状态2
