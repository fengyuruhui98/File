...
label_find_9f0c:		
  ...
  //if((!gM1Size0x53Flag)&&(memcmp(gCityCode,"\x20\x00",2) == 0))  goto label_invalid;//2012/5/11 11:27:31,去掉CPU卡0x53的判断
  	gApplicationTypeTag = outbuf[cnt+8]; //应用类型标识	1
...
//	UDWORD temp2,tint;
	...
	//为了加快速度，将取认证码放在读卡中2018/5/4 10:44
//	memcpy(buf,"\x80\xca\x00\x00\x09",5);
//	ret = mifpro_icmd0(5,buf,outbuf);
//	if(ret<0)
//	{
...
//	set_task_key(TASK_KEY_SEND_AUTHCODE);
  ...
  //memset(&buf[11],0,4);
	...
	{ 
		//昆山/常熟/无锡卡,同时更新0011及0017文件,20131128
		//更新地铁标记，0011文件
		if(update_type == ENTRY_UPDATE)
		{
			gDebugStep = 0x30a5;
			#ifdef _EMU_
					printf("\n昆山/常熟/无锡卡->gUpdateRecord Entry:");
					for(i=0;i<16;i++)  printf(" %02x",rec_buf[i]);
			#endif
			ret = write_record(0x11,0x01,16,rec_buf,0);
			if(ret)
		...
		//3-3	应用锁定标志位	1	HEX－00
		buf[3] = 0; //4-4	复合消费记录版本号	1	HEX
		if(update_type == ENTRY_UPDATE) buf[4] = 1; //5-5	交易类型（0x00标识已完成）	1	HEX
		else buf[4] = 0;
		memcpy(&buf[5],gPosID,6); //6-11	终端机编号（城市代码）	6	BCD
		//buf[11] = *((char *)&gCardThisRemain+0); //12-15	金额（用于补扣）	4	HEX
	...
	else  //非上海卡也非昆山/常熟/无锡卡，更新0017文件
	{
		//update 0017 全国复合消费文件
		...
		//                                //3-3	应用锁定标志位	1	HEX－00
		//  buf[3] = 0; //4-4	复合消费记录版本号	1	HEX
		//  if(gUpdateType == ENTRY_UPDATE)  buf[4] = 1; //5-5	交易类型（0x00标识已完成）	1	HEX
		//     else buf[4] = 0;
		//  memcpy(&buf[5],psam_no,6); //6-11	终端机编号（城市代码）	6	BCD
		//  /*
		//  buf[11] = *((char *)&gCardThisRemain+0); //12-15	金额（用于补扣）	4	HEX
		//  buf[12] = *((char *)&gCardThisRemain+1); 
		//  buf[13] = *((char *)&gCardThisRemain+2); 
		//  buf[14] = *((char *)&gCardThisRemain+3); 
		//  */
		//  if(gUpdateType == ENTRY_UPDATE)  memcpy(&buf[11],"\x00\x00\x02\x58",4);//6元
		...
		//  memcpy(&buf[35],&gUpdateRecord[9],2);//交易金额
		//  memcpy(&buf[37],&gUpdateRecord[12],3);//累计金额
		//  if(gUpdateType == ENTRY_UPDATE)  memcpy(&buf[27],&gUpdateRecord[1],8);//进站时间，进站设备编码
		//	 else memcpy(&buf[40],&gUpdateRecord[1],8);//出站时间，进站设备编码
		...
		memcpy(buf,"\x09\x2e\x00",3); //1-1	复合消费标志	1	BCD－09
		 ...
		 buf[3] = 0; //4-4	复合消费记录版本号	1	HEX
		if(update_type == ENTRY_UPDATE) 
			buf[4] = 1; //5-5	交易类型（0x00标识已完成）	1	HEX
		else 
			buf[4] = 0;
		memcpy(&buf[5],gPosID,6); //6-11	终端机编号（城市代码）	6	BCD
		//buf[11] = *((char *)&gCardThisRemain+0); //12-15	金额（用于补扣）	4	HEX
	...
	buf_to_udword(bgTaskKeyOutbuf,gPosTransCnt);
	memcpy(mac1,&bgTaskKeyOutbuf[4],4);
  //用户卡交易完成,debit for purchase
	memcpy(gBuf,"\x80\x54\x01\x00\x0F",5);
	//gBuf[5] = *((char *)&gPosTransCnt+0);//终端交易流水号	4
...
#ifdef _EMU_
printf("\niso_apdu trans_cpu_update-->用户卡交易完成:");
for (i=0;i<gBufBytes;i++)  printf(" %02x",gBuf[i]);
#endif
		...
		else if(ret == -400)
		{
			...
			{//异地卡
				if(ReadMode==0x90)
				{
					...
					outbuf[22]=transport_17_data[10];//卡类型
					memcpy(&outbuf[23],&transport_15_data[24],4);//有效期
					outbuf[27] = transport_balance[3];//卡余额
					...
					memset(&outbuf[37],0x00,5);//换乘信息：异地卡填0
					cnt=42;
					shmt_prot_send(outbuf,cnt);
