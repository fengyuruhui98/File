...
code UBYTE gCardDir[16]={0x00,0x10,0x01,0x04,0x03,			/*卡上目录信息*/
                         0x03,0x03,0x20,0x21,0x27,
                         0x22,0x23,0x31,0x32,0x33,0x34};         
code UBYTE gCardRecPos[10] = {14,16,17,18,20,21,22,24,25,26};	//普通卡公共记录位置
...
code UBYTE trans_id[14]={0x11,0x11,0x12,0x1f,0x00,0x04,0x26,0x27,0x19,0x35,0x34,0x06,0x07,0x14};	//20070813协议

UBYTE gCPUflag;

UBYTE KeyBuf[60],gB29[20];
UBYTE gRestoreFlag,gClrFlag;					 //	gRestoreFlag:0不需要恢复备份，0x55需要恢复.
													 //gClrFlag:0不需要清除备份, 0x55需要清除备份.
UBYTE save[11];					//本机保护判断数据	
UBYTE gB32[20],gB33[20], gB34[20];		//地铁专用记录缓存	
UBYTE gB32BK[20],gB33BK[20];
UBYTE gBFirst;					//地铁记录提供依据	
UBYTE ChangeDate;				//0x55:需要更改起算日期
UBYTE recordsum[3];				//地铁记录中累积金额
UBYTE recordcnt;				//地铁记录中累积次数
UBYTE recordtime[2];			//地铁记录中累积的判断时间


UBYTE hh_trans_id;			//单程票交易参数
UBYTE b9error;					//B9错误
//
...
//
	...
	if(ret!=0) //增加对0x53的判断,嘉兴市民卡2011-1-19 15:49:00
	{
		#ifdef DEBUG_PRINT
		...
		#ifdef DEBUG_PRINT
		debug_printf("\x0d\x0a M1 mcml_select return %02x!\n", i);
		#endif
...
//	#endif
	...
	gDebugStep=4;
	memset(buf,0,16);
	memcpy(gKeyA,"\xfc\x61\xcb\xb5\x37\x51",6);   //正式
	ret=mcmh_read_simple(1,buf,1,0);
	
	if(ret!=0)
	{ 
		memcpy(gKeyA,"\x70\xa4\xf4\x82\x01\x6d",6);   //测试
	...
	if(!gM1Size0x53Flag)//2011-3-29 10:45:58 M1部分继续拒绝非53标识卡
	{
		printf("gM1Size0x53Flag = %d, !gM1Size0x53Flag= %d\n", gM1Size0x53Flag, !gM1Size0x53Flag);
		goto label_invalid;
	}
...
//	}
	...
	gDebugStep=13;
	if((buf[0] == 0) && (buf[1] == 0) && (buf[2] == 0))		  //交易正常完成
	{		
	  	#ifdef ZD_B30_TEST
			printf("\nzddb30test--0\n");
			#endif
    ...
    gDebugStep=14;
    if((buf[0] == 0x55) && ((UBYTE)buf[1] == (UBYTE)0xaa) &&(buf[2] == 0x55))			//上次交易异常中断，需要用备份内的数据恢复公共信息区的数据，
    {
    																					//即按照备份数据块内的公共信息数据项的内容覆盖公共信息块内的相应内容，
	    gCardRecPtr = buf[7];															//同时要检查钱包数据的一致性，根据备份数据块内钱包金额选择正确的数据块进行恢复。
	    gCardStatus = buf[10];						   									//在数据恢复结束后按交易流程继续进行。
    	if(gCardStatus!=2)
    	{
    		goto label_invalid;
    	}
	...
	printf("\nzddb30test--2\n");
	#endif
	return trans_c();				//没有"55AA55"的交易标志，或备份数据块内校验不正确，
									//则认为上次交易成功完成，无需恢复卡片数据。
									
	#ifdef ZD_B30_TEST
	printf("\nzddb30test--3\n");
	#endif
label_countc_process:
	for(i=1;i<7;i++)						//计次卡密钥调整
	{
...
//	return 1;
	...
	if((buf[0] == 0x55) && (buf[1] ==0xaa) &&(buf[2] == 0x55))		//上次交易异常中断，需要用备份内的数据恢复公共信息区的数据，
    {
    gDebugStep=63;						                        					//即按照备份数据块内的公共信息数据项的内容覆盖公共信息块内的相应内容，
		gCardRecPtr = buf[7];										//同时要检查钱包数据的一致性，根据备份数据块内钱包金额选择正确的数据块进行恢复。
    	gCardStatus = buf[10];	
...
UBYTE trans_a(void)
{
	...
	UBYTE i;
			...
			#endif
	gMetroIoFlagBk = 0;
	gMetroIoFlagBkValid =0;//2022.7.4zddchange 1->0;
	//读B13
	gDebugStep=20;
...
//		}
	...
	*((char *)&gCardPurse2+LONG_HIGH3) = buf[3];
	...
	for(i=0;i<3;i++)
	{
		if(save[i+4]!=*((char *)&gCardPurse2+i))		//比较钱包的低3字节
			goto notsave;
	}
	gCardCnt--;
...
notsave:
	...
	*((char *)&gCardPurse1+LONG_HIGH3) = buf[3];
	...
	if(gCardPurse1 == gCardPurse2) 
	{
		//2011-8-11 10:56:51 更改为以副本block30为主
		if(mcmh_read(30,buf,0,0)!=0)
		{
			goto label_err_read;
		}
...
UBYTE trans_ms(void)
{
	if(gMetroIoFlag==0x55)
	{
		if(lrc_chk2(gB32, 15) != 0)
		{
			//2019/8/19 15:03 新增数据清0
			memset(gB32, 0x00, 16);
			gB32[0] = gB32[15] = 0x41;
			goto trans_ma;
		}
...
UBYTE trans_exit(UBYTE offsetnum)
{
	...
	memcpy(buf,gB32,16);
	gStationTimeOverRideFlag=buf[0];			//普通卡
	*((char *)&tint+INT_LOW) = buf[9];
	*((char *)&tint+INT_HIGH) = buf[10];
	tint2=0;
	*((char *)&tint2+LONG_HIGH0)=buf[10];		//计次卡
...
label_begin:
	...
	save[2]=gThisCardSnr[2];
	save[3]=gThisCardSnr[3];
	save[4]=*((char *)&gCardPurse+LONG_HIGH2); 
	save[5]=*((char *)&gCardPurse+LONG_HIGH1); 
...
label_begin:
	i = (gCardRecPtr+10-gRelativePtr)%10;					//在普通卡中gCardRecPtr为当前记录 
	if(mcmh_read(gCardRecPos[i],record_buf,2,0) != 0)		//在新方案中，计次卡与普通卡的记录结构一样 
	{
    	goto label_err_read;
    }
...
UBYTE trans_update(UBYTE update_type,UBYTE *rec_buf)
{
	...
	//计算TAC
	KeyBuf[0]=0x00;	//充资类设备计算TAC时应使用0，消费类使用0x88
	KeyBuf[1] = *((char *)&gPosTransCnt+LONG_HIGH2);
	KeyBuf[2] = *((char *)&gPosTransCnt+LONG_HIGH1);
	KeyBuf[3] = *((char *)&gPosTransCnt+LONG_HIGH0);
	KeyBuf[4] = gCityCode0;
...
cardrec:
	...
	memcpy(gTAC,bgTaskKeyOutbuf,4);
#ifdef _DEBUG_SHTRANS_
	printf("Update TAC: %02x%02x%02x%02x\n", gTAC[0], gTAC[1], gTAC[2], gTAC[3]);
#endif
	memset(buf,0,16);
	if(mcmh_write(12,buf,2,0) != 0)
	{
...
UBYTE hh_getcardinf(UBYTE ID)
{
	...
	{
		//debug_printf("use B15 backup B9\n");
		//2015/2/3 15:49:04增加B9全0判断
		b9error=0x55;				//b9error=0x55	 校验错误 
	...
	{	
		...
		//debug_printf("B9:00 L %02X S %02X E %02X\n", hh_trans_Line, hh_trans_Station, hh_trans_Equip);
		return 0;
	}
	...
	{	
		...
		//debug_printf("B9:02 L %02X S %02X E %02X\n", hh_trans_Line, hh_trans_Station, hh_trans_Equip);
		return 0;
	}
...
//		}
		...
		//debug_printf("B9:08 L %02X S %02X E %02X\n", hh_trans_Line, hh_trans_Station, hh_trans_Equip);
		return 0;		
	}
...
//			goto label_err_read;
//		}
	  //if(lrc_chk2(hh_Exit,10)!=0)							   //原程序不考虑B14的后两个字节 
		...
		//debug_printf("B9:0C L %02X S %02X E %02X\n", hh_trans_Line, hh_trans_Station, hh_trans_Equip);
		return 0;	
	}
...
/*********************************************************
函数：hh_trans_addvalue
功能：单程票加值
*********************************************************/
UBYTE hh_trans_addvalue()
{
	UBYTE buf[20];
	UWORD i;
	...
	KeyBuf[12]= hh_trans_Line;
	KeyBuf[13]= hh_trans_Station;
	memcpy(&KeyBuf[14],gPosNo,4);       //设备编号-交易设备节点标识码
	KeyBuf[16]= 0x18;
	KeyBuf[18]= *((char *)&gPosTransCnt+LONG_HIGH3);
	KeyBuf[19]= *((char *)&gPosTransCnt+LONG_HIGH2);
	KeyBuf[20]= *((char *)&gPosTransCnt+LONG_HIGH1);
...
/*********************************************************
函数：hh_trans_expiredata
功能：单程票续期
*********************************************************/
UBYTE hh_trans_expiredata(UBYTE *rec_buf)
{
	UBYTE buf[20];
	UWORD i;
...
/*********************************************************
函数：hh_trans_returndata
功能：单程票退票
*********************************************************/
UBYTE hh_trans_returndata()
{
	UBYTE buf[20];
	UWORD i;
...
UBYTE hh_trans_update(UBYTE *rec_buf)
{
	...
	if(updatetype == 0)
	{
		...
		case 0:				
			...
			hh_SaleBuf[10]=rec_buf[18];
			hh_SaleBuf[11]=rec_buf[17];
			break;
		case 1:
			hh_Exit[8]=*((char *)&gCardThisTransValue+LONG_HIGH1);
			hh_Exit[9]=*((char *)&gCardThisTransValue+LONG_HIGH0);
	...
	if(updatetype == 1)
	{
		...
		hh_SaleBuf[0]=gPosNo[0];
		hh_SaleBuf[1]=gPosNo[1];
		hh_SaleBuf[2]=gPosNo[3];
		if(tocken_read(12,&hh_Exit[0])!=0)
		{
			gResponseStatus=CE_READERROR;
			return OP_ERR;
...
//		}
	...
	if(updatetype == 3)
	{
		...
		if(tocken_write(12,&hh_Exit[0])!=0)
		{
			gResponseStatus=CE_WRITEERROR;
			return OP_ERR;
		}
